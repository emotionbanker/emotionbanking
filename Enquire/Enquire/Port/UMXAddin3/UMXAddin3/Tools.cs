...
namespace UMXAddin3
{
    ...
    class Tools
    {
        ...
        {
            float pcnt1 = q.GetAnswerPercentByPerson(0, eval, ps);
            float pcnt2 = q.GetAnswerPercentByPerson(1, eval, ps);
            float pcnt3 = q.GetAnswerPercentByPerson(2, eval, ps);
            float pcnt4 = q.GetAnswerPercentByPerson(3, eval, ps);
            float pcnt5 = q.GetAnswerPercentByPerson(4, eval, ps);
        ...
        public static TargetData Cross(Evaluation eval, TargetData td, Question q, int a)
        {
            try
            {
                ...
                {
                    Question nq = new Question(oq.ID, oq.Text, oq.Display, oq.Answers, oq.Shortcut, oq.NullAnswers);
                    foreach (int uid in UIDs)
                    {
                        Result rs = oq.GetResultByUserID(uid);
                        if (rs != null)
                            nq.Results.Add(new Result(rs.TextAnswer, rs.SelectedAnswer, rs.UserID));
                    }
                    cr.Questions[j++] = nq;
                }
