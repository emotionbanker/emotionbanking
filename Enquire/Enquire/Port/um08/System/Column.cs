...
namespace compucare.Enquire.Legacy.Umfrage2Lib.System
{
	...
	{
	    [NonSerialized] private ILog _logger = LogHelper.GetLogger();
        ...
        {
            _logger.Debug("======================= minpoints for target");
            double tot = 0;
            ...
            foreach (ColumnQuestion cq in Questions)
            {
                ...
                foreach (PersonV pv in cq.gap.Persons)
                {
                    ...
                    {
                        v += MaxGapVal * cq.Weight;
                    }
                }

                //double v = MaxGapVal * ((double)cq.gap.Persons.Count);

                double nonWeighted = v;
                if (cq.Cat != null) v *= ((double)cq.Cat.Weight);

                if (cq.Cat != null) _logger.Debug(cq.SID + ": " + nonWeighted + " is " + v + " for " + cq.Cat.Name);
                tot += v;
            }

            _logger.Debug("======================= minpoints for target : " + tot);
            return tot;
        }
